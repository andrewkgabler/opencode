...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Treat any exit code as success so we can collect stdout regardless
    // (preserves prior behavior: `text` ignored exit code, `run` returned its own code).
    const acceptAnyExit = () => "success" as const
    ...
    const text = Effect.fnUntraced(
      function* (cmd: string[], opts?: { cwd?: string; env?: Record<string, string> }) {
        const result = yield* appProcess.run(
          ...
          { interpretExitCode: acceptAnyExit },
        )
